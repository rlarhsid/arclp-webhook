...
class CommandParser:
    ...
    def get_commands(self, command: bytes):
        ...
        for i in range(
            max(
                bi(self.command[12:16]),
                self.room.players[self.player_index].start_command_num,
            ),
            self.room.command_queue_length,
        ):
    ...
    def command_04(self):
        ...
        flag = 2
        if (
            self.room.players[self.player_index].player_id == self.room.host_id
            and player_id != self.room.host_id
        ):
            for i in range(4):
    ...
    def command_09(self):
        ...
        # 离线判断
        flag_13, player_index_list = self.room.check_player_online(self.s.timestamp)
        ...
        if (
            ...
            and self.room.state in (1, 2)
            and player.player_state == 8
        ):
            # 还在结算给踢了
            # 冗余，为了保险
        ...
        if self.room.is_ready(1, 1) and (
            (self.room.player_num > 1 and not self.room.is_public)
            or (self.room.is_public and self.room.player_num == 4)
        ):
            flag_13 = True
            self.room.state = 2
        ...
        if (
            self.room.state == 1
            and self.room.is_public
            and self.room.player_num > 1
            and self.room.should_next_state
        ):
            flag_0c = True
            flag_13 = True
            self.room.state = 2
        ...
        if player.score.difficulty != self.command[33] and player.player_state not in (
            5,
            6,
            7,
            8,
        ...
        if self.room.is_ready(3, 4) or (
            self.room.state == 3 and self.room.should_next_state
        ):
            flag_13 = True
            flag_0c = True
            self.room.state = 4
        ...
        if self.room.state in (7, 8):
            ...
            if (
                ...
                or player_now_timer == 0
                and player.score.timer != 0
            ):
            ...
            if player.score.timer != 0 or self.room.state != 8:
                for i in self.room.players:
                    i.extra_command_queue.append(self.s.command_0e(self.player_index))
    ...
    def command_23(self):
        ...
        logging.info(f"Player `{player.name}` votes for song `{player.voting}`")
        self.room.command_queue.append(self.s.command_12(self.player_index))
