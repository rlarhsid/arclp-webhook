...
class Player:
    def __init__(self, player_index: int = 0) -> None:
        self.player_id = 0
        self.player_name = (
            b"\x45\x6d\x70\x74\x79\x50\x6c\x61\x79\x65\x72\x00\x00\x00\x00\x00"
        )
        self.token = 0
...
class Room:
    ...
    def get_player_last_score(self):
        ...
        if self.last_song_idx == 0xFFFF:
            return (
                b"\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                * 4
            )
    ...
    def delete_player(self, player_index: int):
        ...
        logging.info(f"Player `{player.name}` leaves room `{self.room_code}`")
    ...
    def is_ready(self, old_state: int, player_state: int):
        ...
        if self.state == old_state:
            for i in self.players:
                if i.player_id != 0 and (
                    i.player_state != player_state or i.online == 0
                ):
                    return False
    ...
    def make_voting(self):
        ...
        if random_list:
            ...
            self.fuck_this = random_list[idx]
            self.selected_voter_player_id = random_list_player_id[idx]
            logging.info(f"Room `{self.room_code}` selected song `{self.fuck_this}`")
            print(f"Room `{self.room_code}` selected song `{self.fuck_this}`")

        else:
            self.random_song()
            logging.info(
                f"Room `{self.room_code}` randomly selected song `{self.fuck_this}`"
            )
            print(f"Room `{self.room_code}` randomly selected song `{self.fuck_this}`")
